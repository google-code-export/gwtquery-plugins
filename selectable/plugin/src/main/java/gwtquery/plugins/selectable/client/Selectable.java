...
public class Selectable extends GQuery {
	...
	private static class Lasso {
		...
		public void show(int leftPosition, int topPosition,
	...
	/**
	 * Give the possibility of each children (depending of the filter option) of each element to be selectable.
	 * 
	 * @return
	 */
	...
	public Selectable unSelectable() {
		for (Element e : elements()) {
			...
			$this.removeClass(CssClassNames.UI_SELECTABLE,
	...
	private GQuery refreshSelectees(Element e) {
		...
		if ((currentOptionsFilter == null && newOptionsFilter != null) || !currentOptionsFilter.equals(newOptionsFilter)){
	...
	protected void onSelectionStart(Element selectable, Event event) {
		...
		GQuery selectedSelectees = $(selectable).data(SELECTEES_KEY,
				GQuery.class).filter("." + CssClassNames.UI_SELECTED);

		for (Element e : selectedSelectees.elements()) {
			SelectableItem si = $(e).data(SELECTABLE_ITEM_KEY,
		...
		//remove old selected (case where filter was change before this selection) if no meta-key was pressed
		if (!(event.getMetaKey() || event.getCtrlKey())){
		...
		for (Element e : parentsAndSelf.elements()) {
			...
			if (si != null) {
				...
				boolean doSelection = !(event.getMetaKey() || event
						.getCtrlKey())
						|| !si.isSelected();
				if (doSelection) {
					si.selecting();
	...
	protected void onSelection(Element selectable, Event event) {
		...
		for (Element e : selectees.elements()) {
			...
			if (Tolerance.TOUCH == options.getTolerance()) {
				...
				hit = si.getLeft() > x1 && si.getRight() < x2
						&& si.getTop() > y1 && si.getBottom() < y2;
			}
				...
				if (si.isSelecting()) {
					if ((event.getMetaKey() || event.getCtrlKey())
						...
						if (si.isStartSelected()) { // previously selected ->
							...
							si.$element()
									.addClass(CssClassNames.UI_UNSELECTING);
	...
	protected void onSelectionStop(Element selectable, Event event) {
		...
		for (Element e : selecting.elements()) {
			SelectableItem si = $(e).data(SELECTABLE_ITEM_KEY,
					SelectableItem.class);			si.select();
			si.setStartSelected(true);
