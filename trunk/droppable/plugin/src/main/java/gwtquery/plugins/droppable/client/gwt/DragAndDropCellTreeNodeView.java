...
import gwtquery.plugins.droppable.client.gwt.extend.com.google.gwt.user.cellview.client.AbstractHasData;
import gwtquery.plugins.droppable.client.gwt.extend.com.google.gwt.user.cellview.client.CellBasedWidgetImpl;
import gwtquery.plugins.droppable.client.gwt.extend.com.google.gwt.user.cellview.client.HasDataPresenter;
import gwtquery.plugins.droppable.client.gwt.extend.com.google.gwt.user.cellview.client.HasDataPresenter.LoadingState;
...
class DragAndDropCellTreeNodeView<T> extends UIObject {
  ...
  private static class NodeCellList<C> implements HasData<C> {
    ...
    private class View implements HasDataPresenter.View<C> {
      ...
      public void render(SafeHtmlBuilder sb, List<C> values, int start,
        ...
        Style style = nodeView.tree.getStyle();
      ...
      protected void addDragAndDropBehaviour(List<C> values, int start) {
        ...
        for (int rowIndex = start; rowIndex < end; rowIndex++) {
          ...
          DragAndDropCellWidgetUtils.get().maybeMakeDraggableOrDroppable(
              ...
              nodeView.tree.ensureDragAndDropHandlers());
        }
      ...
      private void loadChildState(List<C> values, int start,
        ...
        for (int i = start; i < end; i++) {
          ...
          if (savedChild != null) {
            ...
            if (child.children != null) {
              for (DragAndDropCellTreeNodeView<?> grandchild : child.children) {
        ...
        // node.
        DragAndDropCellTreeNodeView<?> curNode = keyboardSelected;
        while (curNode != null) {
          if (curNode == nodeView) {
      ...
      private Map<Object, DragAndDropCellTreeNodeView<?>> saveChildState(
        ...
        ProvidesKey<C> keyProvider = nodeInfo.getProvidesKey();
        Map<Object, DragAndDropCellTreeNodeView<?>> savedViews = new HashMap<Object, DragAndDropCellTreeNodeView<?>>();
        for (C childValue : values) {
          ...
          DragAndDropCellTreeNodeView<?> savedView = openNodes.remove(key);
          if (savedView != null) {
        ...
        for (DragAndDropCellTreeNodeView<?> lostNode : openNodes.values()) {
  ...
  private static class TreeNodeImpl implements TreeNode {
    ...
    public TreeNodeImpl(DragAndDropCellTreeNodeView<?> nodeView) {
      this.nodeView = nodeView;
    }
    ...
    public TreeNode setChildOpen(int index, boolean open, boolean fireEvents) {
      assertNotDestroyed();
      checkChildBounds(index);
      DragAndDropCellTreeNodeView<?> child = nodeView.getChildNode(index);
   ...
   */
  private List<DragAndDropCellTreeNodeView<?>> children;
   ...
   */
  private DragAndDropCellTreeNodeView<?> parentNode;
  ...
  /**
   * The {@link DragAndDropCellTree} that this node belongs to.
   */
  ...
  public DragAndDropCellTreeNodeView<?> getChildNode(int childIndex) {
  ...
  protected void cleanup(boolean destroy) {
    ...
    if (children != null) {
      for (DragAndDropCellTreeNodeView<?> child : children) {
        child.cleanup(true);
      }
      children = null;
    }
  ...
  protected void fireEventToCell(NativeEvent event) {
    ...
    // Update selection.
    if (selectionModel != null && isClick && !parentCell.handlesSelection()) {
   ...
   */
  DragAndDropCellTreeNodeView<?> getParentNode() {
    return parentNode;
  }
  ...
  int indexOf(DragAndDropCellTreeNodeView<?> child) {
